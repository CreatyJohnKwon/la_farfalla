"use client";

import Image from "next/image";
import Chillguy from "@/public/chill.png"
import Link from "next/link";

const MainScreen = () => {
    return (
        <div className="flex flex-col items-center justify-center min-h-screen w-screen overflow-hidden">
            <div className="relative w-1/2 h-1/2 font-serif text-start c_md:text-center text-4xl c_base:text-7xl c_md:text-9xl text-black -mt-72 transition-all duration-300 ease-in-out">
                <Image
                    src={Chillguy}
                    alt="chill_guy_img"
                    width={500}
                    height={500}
                    className="absolute inset-0 -z-50 blur-md object-cover place-self-center"
                />
                <div className="-ms-10 c_base:ms-0">
                    <Link href="/menu">
                        <p>LA_FARFALLA</p>
                        <p>WE ARE CHILL</p>
                    </Link>
                </div>
            </div>
        </div>
    )
}

export default MainScreen;