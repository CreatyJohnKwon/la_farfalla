...
const ShopClient = () => {
    ...
    const [searchFilteredProducts, setSearchFilteredProducts] = useState<Product[]>( // 타입 명확화
        [],
    );
    ...
    const handleRealTimeSearch = useCallback(
        (query: string, searchResults: Product[]) => { // searchResults 타입 명확화
    ...
    // 컴포넌트 마운트 시 이미지 프리로드
    useEffect(() => { // useMemo 대신 useEffect로 변경
        if (
            !productsLoading &&
