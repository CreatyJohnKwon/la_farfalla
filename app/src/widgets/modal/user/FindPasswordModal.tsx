...
const FindPasswordModal = ({ onClose }: { onClose: () => void }) => {
    ...
    return (
        <ModalWrap
            onClose={onClose}
            className="relative w-[90vw] rounded-sm bg-white p-6 shadow-xl md:w-[40vw]"
        >
            <button
                onClick={onClose}
                className="absolute right-4 top-5 text-xl font-amstel font-[500] text-gray-500 hover:text-gray-900 sm:text-2xl"
            >
                &times;
            </button>
            <h2 className="mb-4 text-start text-lg font-bold sm:text-xl">비밀번호 찾기</h2>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <input
                    ...
                    placeholder="이메일 주소"
                    className="w-full border border-gray-200 px-4 py-3 text-sm text-black placeholder:text-gray-400 focus:outline-none sm:text-base"
                    required
                />
                <button
                    type="submit"
                    disabled={isPending || email.length === 0}
                    className={`w-full py-3 font-pretendard text-sm text-white transition-colors sm:text-base
            ...
            {isSuccess && (
                <div className="mt-4 bg-green-100 p-3 text-xs text-green-700 sm:text-sm">
