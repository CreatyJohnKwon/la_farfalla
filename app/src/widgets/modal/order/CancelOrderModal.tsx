...
const CancelOrderModal = ({
    ...
    const handleSubmit = async () => {
        if (!reason.trim()) {
            // alert("사유를 입력해주세요."); // Toast/Snackbar 등으로 대체하는 것을 권장
            console.warn("Reason is required");
            return;
        }
        ...
        try {
            ...
            // 여기에 사용자에게 보여줄 에러 처리 UI(예: Toast)를 추가할 수 있습니다.
        } finally {
            onClose();
            setReason("");
    ...
    return (
        <ModalWrap
            onClose={onClose}
            className="w-full max-w-md rounded-lg bg-white p-0" // 패딩은 내부에서 제어하므로 p-0 추가
        >
            {/* 헤더 */}
            ...
            <div className="space-y-4 p-4">
                        ...
                        <div className="space-y-2">
                            <label className="flex items-center">
                                <input
                                    ...
                                    onChange={(e) => setRequestType(e.target.value as "exchange")}
                                    className="mr-2"
                                />
                                <span className="font-pretendard text-sm text-gray-700">교환 요청</span>
                            </label>
                            <label className="flex items-center">
                                <input
                                    type="radio"
                                    ...
                                    className="mr-2"
                                />
                                <span className="font-pretendard text-sm text-gray-700">반품 요청</span>
                            </label>
                        </div>
                    </div>
                )}
                ...
                <div>
                    <label className="mb-2 block font-pretendard text-sm font-medium text-gray-700">
                        {actionText()} 사유<span className="text-red-500">*</span>
                    </label>
                    <textarea
                        value={reason}
                ...
                <div className="rounded-sm border border-yellow-200 bg-yellow-50 p-3">
                    <div className="text-sm text-yellow-800">
                        <p className="mb-1 font-pretendard font-medium">처리 안내</p>
