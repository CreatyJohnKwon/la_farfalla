        ...
        >
            ...
            <div className="space-y-6 text-gray-700">
                    ...
                    {Object.entries(statusResult).map(([key, label]) => {
                        if (key === "return" || key === "exchange" || key === "prepare") {
                            ...
                            return (
                                <label
                                    key={key}
                                    className="flex cursor-pointer items-center space-x-4 rounded-sm p-3 transition hover:bg-gray-100"
                                >
                                    <input
                                        type="radio"
                                        name="orderStatus"
                ...
                {radioValue === "shipped" && (
                    <div className="w-full flex-shrink-0 flex-col">
                        ...
                        <input
                            ...
                            className="w-full rounded-sm border border-gray-300 p-2 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-300"
                ...
                {radioValue === "confirm" && (
                    <div className="relative w-full flex-shrink-0 flex-col">
                        ...
                        <h1
                            className={`"text-black"} w-full rounded-sm border border-gray-300 p-2`}
                        >
                            {orderData?.trackingNumber
                                ? orderData?.trackingNumber
                                : "아직 출고 전입니다."}
            ...
            <div className="mt-6 flex gap-3 text-lg text-white">
                <button
                    ...
                    className="w-full rounded-sm bg-gray-800 py-2 hover:bg-gray-700"
                >
                    변경
                </button>
                <button
                    onClick={onClose}
                    className="w-full rounded-sm bg-slate-200 py-2 text-black hover:bg-slate-100"
                >
                    닫기
                </button>
            </div>
