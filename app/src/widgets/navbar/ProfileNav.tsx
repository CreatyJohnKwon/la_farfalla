...
const ProfileNavbar = ({ id }: { id: string }) => {
    return (
        <nav className="absolute top-12 h-full w-full items-center sm:items-start transition-all duration-300 ease-in-out sm:static sm:top-20 sm:col-span-2 flex flex-col">
            <ul className="font-amstel flex w-[90vw] flex-row justify-between sm:justify-evenly pt-[4vh] text-sm transition-all duration-300 ease-in-out sm:flex-col sm:ps-8 sm:pt-36 sm:text-base md:ps-10 md:text-xl">
                {profileNavData.map((navList, index) => (
                    ...
                    >
                        <button
                            className={`z-40 ${navList.id.toLowerCase() === id ? "text-black/50 decoration-1" : "text-black"}`}
